 ...
 * verbatim — VRAM, temperature, whether Ollama answered.
 */
...
export function EngineBadge({ engine, reason, hardware, onOverride, disabled }: EngineBadgeProps) {
  const [open, setOpen] = useState(false);

  const isLocal = engine === "local";
  const gpu = hardware?.gpus?.[0];

  const label = engine === null
    ? "Detecting hardware…"
    : isLocal
      ? `Local Engine${gpu ? ` · ${shorten(gpu.name)}` : hardware?.apple_silicon ? " · Apple Silicon" : ""}`
      : "Cloud Accelerated · Groq";
      ...
      >
        <span
          className={[
            ...
            engine === null ? "bg-haze-400" : isLocal ? "bg-emerald-400" : "bg-sky-400",
          ].join(" ")}
        />
        ...
        <div className="mt-1.5 animate-fade-up space-y-3 rounded-lg border border-white/[0.06] bg-ink-800/50 p-3 text-[12px]">
          ...
          {hardware && (
            <dl className="space-y-1 font-mono text-[11px] text-haze-400">
              ...
              <Row label="Ollama" value={hardware.ollama_reachable ? "reachable" : "unreachable"} />
            ...
            <div className="flex gap-1 border-t border-white/[0.06] pt-2.5">
              {(["auto", "local", "cloud"] as EnginePreference[]).map((pref) => (
                <button
                  ...
                  className="flex-1 rounded-md border border-white/[0.06] px-2 py-1 text-[11px] capitalize text-haze-300 transition-colors hover:bg-white/[0.06] disabled:cursor-not-allowed disabled:opacity-40"
                >
                  {pref}
                </button>
              ))}
